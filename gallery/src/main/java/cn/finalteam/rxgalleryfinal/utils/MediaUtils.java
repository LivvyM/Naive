...
public class MediaUtils {
     ...
     * @return
     */
    public static List<MediaBean> getMediaWithImageList(Context context, String bucketId, int page, int limit,boolean isCache) {
        ...
        if(cursor != null) {
            ...
            if(count > 0) {
                cursor.moveToFirst();
                do {
                    MediaBean mediaBean = parseImageCursorAndCreateThumImage(context, cursor,isCache);
    ...
    public static MediaBean getMediaBeanWithImage(Context context, String originalPath) {
        ...
        if(cursor != null && cursor.getCount() > 0) {
            cursor.moveToFirst();
            mediaBean =  parseImageCursorAndCreateThumImage(context, cursor,false);
        }
        if(cursor != null && !cursor.isClosed()){
            cursor.close();
        }
     ...
     * @return
     */
    private static MediaBean parseImageCursorAndCreateThumImage(Context context, Cursor cursor,boolean isCache) {
        ...
        if(isCache){
            ...
            mediaBean.setThumbnailBigPath(originalPath);
            mediaBean.setThumbnailSmallPath(originalPath);
        }
