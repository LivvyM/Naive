...
import cc.livvy.naive.Constant;
import cc.livvy.naive.DemoHelper;
import cc.livvy.naive.MainActivity;
import cc.livvy.naive.R;
import cc.livvy.naive.domain.EmojiconExampleGroupData;
import cc.livvy.naive.domain.RobotUser;

public class PPTChatFragment extends EaseChatFragment implements EaseChatFragmentHelper {
...
//            inputMenu.hideExtendMenuContainer();
    ...
    public void onSendMessageComplete() {
        EventBus.getDefault().post(new ChatEventBus(ChatEventBus.OPT_SEND_MESSAGE));
    }

    @Override
    public void onJoinComplete() {
        EventBus.getDefault().post(new ChatEventBus(ChatEventBus.OPT_JOIN_ROOM));
    }

    @Override
    public void onVerifyPermission(boolean isOwner) {
        EventBus.getDefault().post(new ChatEventBus(ChatEventBus.OPT_PERMISSION, isOwner));
    }

    @Override
    public void onMessageReceived() {
        EventBus.getDefault().post(new ChatEventBus(ChatEventBus.OPT_RECEIVE));
    }
...
//                }
                ...
                if (RedPacketUtil.isRandomRedPacket(message)) {//小额随机红包
